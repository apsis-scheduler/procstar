...
def _get_local(ws_server):
    ...
    return (
        ...
        if s.type == socket.SOCK_STREAM and s.family in (socket.AF_INET, socket.AF_INET6)
    )
...
class Assembly:
    ...
    def _build(self, conn_id, group_id, access_token, args):
        ...
        """
        token = self.access_token if access_token is DEFAULT else access_token
        return (
            [
                get_procstar_path(),
                "--agent",
    ...
    async def start_procstars(self, counts, *, access_token=DEFAULT, args=[]):
        ...
        """
        conns = set((g, str(uuid.uuid4())) for g, n in counts.items() for _ in range(n))
        procs = set()
        ...
        with self.server.connections.subscription() as events:
            ...
            try:
                ...
                if res is not None:
                    ...
                    raise ProcstarError(f"procstar process failed with {res} before connecting")
            finally:
                for task in tasks:
                    task.cancel()
    ...
    def stop_instances(self):
        ...
        return asyncio.gather(*(self.stop_instance(c) for c in conn_ids))
    ...
    async def run(self, spec) -> Result:
        ...
        return (await self.wait(proc)) if res.state == "running" else res
